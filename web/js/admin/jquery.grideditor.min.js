    ...
    $.fn.gridEditor = function (options) {
        ...
        self.each(function (baseIndex, baseElem) {
            ...
            var canvas,
                ...
                htmlTextArea
            ;
            var colClasses = ['col-xxl-', 'col-xl-', 'col-lg-', 'col-md-', 'col-sm-', 'col-'];
            ...
            function setup() {
                ...
                $.each(settings.new_row_layouts, function (j, layout) {
                    var btn = $('<a class="btn btn-xs btn-primary" />')
                        .attr('title', 'Dodaj rząd ' + layout.join('-'))
                        .on('click', function () {
                    ...
                    layout.forEach(function (i) {
                        icon += '<div class="column col col-sm-' + i + '"/>';
                    });
                    icon += '</div>';
                    btn.append(icon);
                });
                ...
                var layoutDropdown = $('<div class="dropdown pull-right ge-layout-mode">' +
                    '<button type="button" class="btn btn-xs btn-primary dropdown-toggle" data-toggle="dropdown"><span>Desktop (1320px)</span></button>' +
                    '<ul class="dropdown-menu" role="menu">' +
                    '<li><a data-width="auto" title="Desktop  xxl"><span>Desktop  (1320px)</span></a></li>' +
                    '<li><a title="Desktop xl"><span>Desktop (1140px)</span></a></li>' +
                    '<li><a title="Tablet lg"><span>Tablet (960px)</span></li>' +
                    '<li><a title="Tablet md"><span>Tablet (720px)</span></li>' +
                    '<li><a title="Phone sm"><span>Telefon (540px)</span></a></li>' +
                    '<li><a title="Phone xs"><span>Telefon (auto)</span></a></li>' +
                    '</ul>' +
                    '</div>')
                    .on('click', 'a', function () {
                        var a = $(this);
                ...
                var btnGroup = $('<div class="btn-group pull-right"/>')
                    .appendTo(wrapper)
                ;
                var htmlButton = $('<button title="Edycja źródła" type="button" class="btn btn-xs btn-primary gm-edit-mode"><span class="glyphicon glyphicon-chevron-left"></span><span class="glyphicon glyphicon-chevron-right"></span></button>')
                    .on('click', function () {
            ...
            function createColControls() {
                ...
                canvas.find('.column').each(function () {
                    ...
                    createTool(drawer, 'Properties', '', 'glyphicon-edit', function () {
                        colProperties.toggle();
                    });


                    createTool(drawer, 'Color picker', '', 'glyphicon-tint', function () {


                        drawer.parent('.column').find('.colorsTint').minicolors({
                            format: 'rgba',
                            control: 'hue',
                            opacity: true,
                            theme: 'bootstrap',
                            ...
                            change: function (value, opacity) {
                                ...
                                drawer.parent('.column').find('.ge-content').css({'backgroundColor': value});
                    ...
                    var colProperties = createPropertiesDetails(col).appendTo(drawer);

                    var colTint = createTintDetails(col).appendTo(drawer);
            ...
            function createTintDetails(container){

                var wrapper = container.parent().find('.ge-content');
                var detailsDiv = $('<div class="ge-details" />');

                var inpt = $('<input size="25" class="form-control colorsTint" type="text" />').val((wrapper !== undefined)?wrapper.css('backgroundColor'):'')

                inpt.appendTo(detailsDiv);

                return detailsDiv;
            }

            function createPropertiesDetails(container) {
                ...
                if(container.attr('id')){
                    idToken = container.attr('id')
                    container.attr('id', idToken);
                }else{
                    idToken = 'cms'+rand;
                    container.attr('id', idToken);
                }
                ...
                var cols = [];
                cols.push(createInputProperty(container,'property-desktop-pl','Desktop padding left (px)'));
                cols.push(createInputProperty(container,'property-desktop-pr','Desktop padding right (px)'));

                cols.push(createInputProperty(container,'property-desktop-pt','Desktop padding top (px)'));
                cols.push(createInputProperty(container,'property-desktop-pb','Desktop padding bottom (px)'));

                cols.push(createInputProperty(container,'property-desktop-height','Wysokość desktop'));

                cols.forEach(function(col){
                    col.appendTo(row)
                });

                row.appendTo(detailsDiv);



                cols = [];
                row = $('<div class="row" />');
                cols.push(createInputProperty(container,'property-tablet-pl','Tablet padding left(px)'));
                cols.push(createInputProperty(container,'property-tablet-pr','Tablet padding right (px)'));

                cols.push(createInputProperty(container,'property-tablet-pt','Tablet padding top (px)'));
                cols.push(createInputProperty(container,'property-tablet-pb','Tablet padding bottom (px)'));

                cols.push(createInputProperty(container,'property-tablet-height','Wysokość tablet'));
                ...
                cols = [];
                row = $('<div class="row" />');
                cols.push(createInputProperty(container,'property-phone-pl','Phone padding left (px)'));
                cols.push(createInputProperty(container,'property-phone-pr','Phone padding right (px)'));

                cols.push(createInputProperty(container,'property-phone-pt','Phone padding top (px)'));
                cols.push(createInputProperty(container,'property-phone-pb','Phone padding bottom (px)'));

                cols.push(createInputProperty(container,'property-phone-height','Telefon wysokość'));
            ...
            function createInputProperty(container, key, labelText) {

                var col = $('<span class="col-sm-3 col" />');
                var label = $('<label  class="ge-label">'+labelText+'</label>');
                $('<input class="form-control ge-'+key+'" />')
                    .attr('placeholder', labelText)
                    .val(container.attr('data-'+key))
                    ...
                    .change(function () {
                        ...
                        if(isNaN(parseInt(this.value))){
                            ...
                            container.attr('data-'+key, this.value);
                        }
            ...
            function makeSortable() {
                canvas.find('.row').sortable({
                    ...
                    update: function (event, ui) {
                        var $lis = $(this).children('.column');
                        $lis.each(function () {
                            var colContext = colClasses[curColClassIndex];
                            var currWidth = colContext.replace('col-', '');

                            var orderCurrWidthTpl = 'order-' + currWidth;

                            $(this).removeClass(function (index, className) {

                                // (/(^|\s)"orderCurrWidthTpl"+\S+"/g)
                                // (/(^|\s)order-\S+/g)
                                re = new RegExp("\\b" + orderCurrWidthTpl + "\\d+", 'g');
                                return (className.match(re) || []).join(' ');
                            });


                            $(this).addClass(orderCurrWidthTpl + $(this).index());
            ...
            function switchLayout(colClassIndex) {
                ...
                var layoutClasses = [
                    'ge-layout-desktop-xxl',
                    'ge-layout-desktop-xl',
                    'ge-layout-tablet-lg',
                    'ge-layout-tablet-md',
                    'ge-layout-phone-sm',
                    'ge-layout-phone-xs'
